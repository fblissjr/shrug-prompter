...
## QwenImageWanBridge Integration

The workspace also contains the ComfyUI-QwenImageWanBridge project for direct latent bridging between Qwen-Image and WAN video generation models.

### Key Bridge Nodes (Production)

Located in `/Users/fredbliss/workspace/ComfyUI-QwenImageWanBridge/`:

1. **QwenWANUnifiedI2V** (`nodes/qwen_wan_unified_i2v.py`)
   - Complete I2V solution with all options
   - 5 i2v modes: standard, direct_latent, reference, hybrid, vace_style
   - 5 noise modes: no_noise, add_noise, mix_noise, scaled_noise, decay_noise
   - Handles WAN 2.1 (16ch) vs 2.2 (48ch) automatically
   - Sophisticated channel expansion and normalization

2. **QwenWANUnifiedT2V** (`nodes/qwen_wan_unified_t2v.py`)
   - T2V/V2V generation with optional Qwen guidance
   - 5 generation modes: t2v, t2v_guided, v2v, v2v_guided, latent_morph
   - Multiple noise schedules
   - Temporal consistency control

3. **Utility Nodes** (`nodes/qwen_wan_i2v_bridge.py`)
   - QwenWANI2VBridge: Creates I2V conditioning
   - QwenToImage: Decodes Qwen latent to image
   - QwenWANI2VDirect: Direct latent transfer

### Critical Discoveries

- **WAN 2.1**: 16 channels (compatible with Qwen-Image)
- **WAN 2.2**: 48 channels (requires channel expansion)
- Direct latent transfer partially works but with quality degradation
- Best results with WAN 2.1 models due to matching channel count

### Usage Pattern

```
[Load Image] → [Qwen2VLFlux Encode] → [QwenWANUnifiedI2V] → [KSampler] → [VAE Decode]
```

Key: Connect ALL THREE outputs from unified node (positive, negative, latent) to KSampler.

### Bridge Architecture Notes

- Works with native ComfyUI only (not Kijai's wrapper)
- Avoids VAE decode/encode cycles for better quality
- Includes WAN normalization (mean=0.0, std=0.5)
- Frequency-based channel expansion for WAN 2.2
- No emojis in code or display names

## Qwen-Image Edit Templates

The workspace includes specialized templates for qwen-image edit model (qwen2.5-vl) in `/templates/`:

### Core Edit Templates
- **qwen_image_edit_rewriter.md** - Professional instruction rewriter optimized for dataset patterns
- **qwen_image_edit_structured.md** - Structured templates with system prompts
- **qwen_image_batch_processor.md** - Batch processing with adaptive rules
- **qwen_image_simple_prompts.md** - Quick reference for common edits
- **qwen_image_diverse_examples.md** - 30 real-world examples with diverse scenarios
- **qwen_image_structured_schemas.md** - YAML/attribute-based schemas for precise control

### Bilingual Templates (English/Chinese)
- **qwen_image_en_to_zh.md** - English input → Chinese structured output
- **qwen_image_diffsynth_bridge.md** - DiffSynth-Studio patterns for bilingual workflows
- **qwen_image_photo_edit_zh.md** - General-purpose Chinese photo editing (人像美化、风景优化等)

### Template Usage Notes
All templates are designed as system prompts for the LLM, following these patterns:
- Text must always be in quotes: "Example Text"
- Positions use grid system (top-left, center, bottom-right)
- Sizes in pixels or percentages
- Colors as names or hex codes
- Instructions under 50 words
- Explicit preservation context

### Key Documentation
- **QWEN_IMAGE_ANALYSIS.md** - Dataset analysis and patterns discovered
- **QWEN_IMAGE_USAGE_GUIDE.md** - Complete implementation guide with workflows

### New Chinese Language Templates (2025)
- **qwen_image_en_to_zh.md** - English to Chinese translation with structured output
- **qwen_image_diffsynth_bridge.md** - DiffSynth-Studio integration for bilingual workflows
- **qwen_image_photo_edit_zh.md** - General-purpose Chinese photo editing commands
