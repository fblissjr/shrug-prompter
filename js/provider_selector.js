        ...
        if (nodeData.name === "ShrugProviderSelector") {
            ...
            nodeType.prototype.onNodeCreated = function () {
                ...
                this._updateModelWidget = (models, selectedValue = null) => {
                    ...
                    if (selectedValue) {
                        ...
                        if (models.includes(currentValue)) {
                            ...
                            this.llmModelWidget.value = visionModels.length > 0 ? visionModels[0] : models[0];
                        }
                    }
                };
                ...
                this._logSuccess = (models) => {
                    const visionCount = models.filter(m => m.includes("(Vision)")).length;
                    const statusMsg = visionCount > 0 
                ...
                try {
                    ...
                    console.log("Shrug: Enhanced provider selector initialized successfully");

                } catch (error) {
                    console.error("Shrug: Initialization error:", error);
                    // Graceful degradation - at least show the basic widget
                    if (this.llmModelWidget) {
                        this._updateModelWidget(["Initialization failed"], "Initialization failed");
                    }
                }
            };
        }
