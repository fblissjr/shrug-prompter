...
VACE_PROMPT_TEMPLATE = """You are a prompt optimization specialist whose goal is to rewrite the user's input prompts into high-quality English prompts by referring to the details of the user's input images, making them more complete and expressive while maintaining the original meaning. You need to integrate the content of the user's photo with the input prompt for the rewrite, strictly adhering to the formatting of the examples provided.
...
class LLMPromptGenerator:
    """
    A ComfyUI node that uses the Gemini API to generate a descriptive prompt
    for the next video segment, specifically tailored for the Wan2.1 VACE model.
    """
    def __init__(self):
        self.client = None
        ...
        if api_key:
            try:
                # Use the genai.configure() method which is simpler for API key setup
                genai.configure(api_key=api_key)
                self.client_initialized = True
                print("Gemini Client configured successfully via API key.")
            except Exception as e:
                self.client_initialized = False
                print(f"Failed to configure Gemini Client: {e}")
        else:
            self.client_initialized = False
            print("WARNING: GEMINI_API_KEY environment variable not set. LLMPromptGenerator will not work.")
